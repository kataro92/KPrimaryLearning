...
import type { NumberMcqChallenge } from './questions';
import { generateQuestions, timePerQuestionMs } from './questions';
import { TowerScene } from './towerScene';

export type { PlayResult };

export function renderThapTrieuSoGame(
  root: HTMLElement,
  sceneHost: SceneHost,
  onDone: (result: PlayResult) => void
): () => void {
  const gameId = 'thap-trieu-so';
  ...
  const stage = createGameStage(root, sceneHost, gameId, 'game-play--trieu-so');
  sceneHost.setParallaxSway(false);

  const heroHost = stage.root.querySelector<HTMLElement>('#game-hero')!;
  heroHost.innerHTML = `
    <div class="trieu-so-hero">
      <div class="trieu-so-hero__tower" id="trieu-so-tower"></div>
      <p class="trieu-so-hero__caption" id="trieu-so-status">Tháp Triệu Số · 0/${questions.length} tầng</p>
    </div>
  `;
  const towerMount = heroHost.querySelector<HTMLElement>('#trieu-so-tower')!;
  const statusEl = heroHost.querySelector<HTMLElement>('#trieu-so-status')!;
  const towerScene = new TowerScene(towerMount, questions.length);

  let index = 0;
  let correctCount = 0;
  let questionStarted = Date.now();
  let locked = false;
  let current: NumberMcqChallenge | null = null;

  const updateStatus = () => {
    statusEl.textContent = `Tháp Triệu Số · ${correctCount}/${questions.length} tầng`;
  };
  ...
  const submit = (choiceIndex: number) => {
    ...
    if (ok) {
      correctCount++;
      towerScene.onCorrectAnswer();
    } else {
      towerScene.onWrongAnswer();
    }
  ...
  const showQuestion = (q: NumberMcqChallenge) => {
    current = q;
    locked = false;
