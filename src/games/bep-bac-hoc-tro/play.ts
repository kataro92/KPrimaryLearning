...
import type { FracOpsMcqChallenge } from './questions';
import { generateQuestions, timePerQuestionMs } from './questions';

export type { PlayResult };

export function renderBepBacHocTroGame(
  root: HTMLElement,
  sceneHost: SceneHost,
  onDone: (result: PlayResult) => void
): () => void {
  const gameId = 'bep-bac-hoc-tro';
  ...
  const stage = createGameStage(root, sceneHost, gameId, 'game-play--bep-bac');
  sceneHost.setParallaxSway(false);

  const heroHost = stage.root.querySelector<HTMLElement>('#game-hero')!;
  heroHost.innerHTML = `
    <div class="bep-bac-hero">
      <div class="bep-bac-hero__kitchen" id="bep-bac-kitchen"></div>
      <p class="bep-bac-hero__caption" id="bep-bac-status">Bếp Bác Học Trò · 0/${questions.length} bước</p>
    </div>
  `;
  const kitchenMount = heroHost.querySelector<HTMLElement>('#bep-bac-kitchen')!;
  const statusEl = heroHost.querySelector<HTMLElement>('#bep-bac-status')!;
  const kitchenScene = new KitchenScene(kitchenMount, questions.length);

  let index = 0;
  let correctCount = 0;
  let questionStarted = Date.now();
  let locked = false;
  let current: FracOpsMcqChallenge | null = null;

  const updateStatus = () => {
    statusEl.textContent = `Bếp Bác Học Trò · ${correctCount}/${questions.length} bước`;
  };
  ...
  const submit = (choiceIndex: number) => {
    ...
    if (ok) {
      correctCount++;
      kitchenScene.onCorrectAnswer();
    } else {
      kitchenScene.onWrongAnswer();
    }
  ...
  const showQuestion = (q: FracOpsMcqChallenge) => {
    current = q;
    locked = false;
