...
import { LandPlotScene } from './landPlotScene';
import type { AreaMcqChallenge } from './questions';
import { generateQuestions, timePerQuestionMs } from './questions';

export type { PlayResult };

export function renderDoDatCoThanhGame(
  root: HTMLElement,
  sceneHost: SceneHost,
  onDone: (result: PlayResult) => void
): () => void {
  const gameId = 'do-dat-co-thanh';
  ...
  const stage = createGameStage(root, sceneHost, gameId, 'game-play--co-thanh');
  sceneHost.setParallaxSway(false);

  const heroHost = stage.root.querySelector<HTMLElement>('#game-hero')!;
  heroHost.innerHTML = `
    <div class="co-thanh-hero">
      <div class="co-thanh-hero__plot" id="co-thanh-plot"></div>
      <p class="co-thanh-hero__caption" id="co-thanh-status">Đo Đất Cổ Thành · 0/${questions.length} ô</p>
    </div>
  `;
  const plotMount = heroHost.querySelector<HTMLElement>('#co-thanh-plot')!;
  const statusEl = heroHost.querySelector<HTMLElement>('#co-thanh-status')!;
  const landScene = new LandPlotScene(plotMount, questions.length);

  let index = 0;
  let correctCount = 0;
  let questionStarted = Date.now();
  let locked = false;
  let current: AreaMcqChallenge | null = null;

  const updateStatus = () => {
    statusEl.textContent = `Đo Đất Cổ Thành · ${correctCount}/${questions.length} ô`;
  };
  ...
  const submit = (choiceIndex: number) => {
    ...
    if (ok) {
      correctCount++;
      landScene.onCorrectAnswer();
    } else {
      landScene.onWrongAnswer();
    }
  ...
  const showQuestion = (q: AreaMcqChallenge) => {
    current = q;
    locked = false;
